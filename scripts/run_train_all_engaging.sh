...
#
# Weights land in:  weights/v2_<scale>_<arch>.bytes
# Training CSVs in: weights/v2_<scale>_<arch>_training_profile.csv
# Step ckpts in:    weights/v2_<scale>_<arch>_step<N>.bytes  (every 2000 steps)
#
...
python3 -u python/train_configs.py --index "${SLURM_ARRAY_TASK_ID}"
