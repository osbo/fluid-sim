...
def _default_weights(arg: Optional[str]) -> Path:
    ...
    cand = SCRIPT_DIR.parent / "weights" / "v2_8192_d128_L3_hw.bytes"
    if cand.exists():
        return cand
    streamed = SCRIPT_DIR.parent / "Assets" / "StreamingAssets" / "leaf_only_weights_sim_8192.bytes"
    if streamed.exists():
        return streamed
...
def _default_data(arg: Optional[str]) -> Path:
    if arg:
        return Path(arg).expanduser().resolve()
    return SCRIPT_DIR.parent / "data" / "multiphase_v2_8192" / "test"
...
def main() -> int:
    ...
    ap.add_argument("--output-dir", type=str,
                    default=str(SCRIPT_DIR.parent / "results" / "baseline_benchmark"),
