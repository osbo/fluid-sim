...
def main():
    ...
    p.add_argument(
        "--weights",
        default=str(Path(__file__).resolve().parent.parent / "weights" / "v2_16384_d128_L3_hw.bytes"),
    ...
    p.add_argument(
        "--out-video",
        default=str(Path(__file__).resolve().parent.parent / "results" / "leafonly_inverse_layer_build_16384.mp4"),
    )
    args = p.parse_args()
