...
% Placeholder for visuals that will be supplied later (rendered hero shots, modality grid).
...
that delivers interactive framerates across the full range. It runs at $\sim\!154$\,fps at
...
\begin{teaserfigure}
  ...
  \begin{minipage}[t]{0.245\linewidth}\centering
    \figplaceholder{\linewidth}{1.65in}{Hero render: stiff 3D multiphase
      pressure-Poisson scene driven at interactive rate by our preconditioner.
      Volumetric rendering, heavy/light fluid interfaces, dramatic lighting.}\\[1pt]
    {\footnotesize (a) Hero result}
  \end{minipage}\hfill
  \begin{minipage}[t]{0.245\linewidth}\centering
    \figplaceholder{\linewidth}{1.65in}{Hardest test case: closed-barrier, near-disconnected
      sub-domain, $100\!\times$ contrast --- the configuration where local preconditioners
      stall completely.}\\[1pt]
    {\footnotesize (b) Hardest configuration}
  \end{minipage}\hfill
  \begin{minipage}[t]{0.245\linewidth}\centering
    \figplaceholder{\linewidth}{1.65in}{Stylized beauty shot: multiphase pressure-Poisson
      run as a film-style render, vorticity / Schlieren visualisation.}\\[1pt]
    {\footnotesize (c) Visual showcase}
  \end{minipage}\hfill
  \begin{minipage}[t]{0.245\linewidth}\centering
    \includegraphics[width=\linewidth,height=1.65in,keepaspectratio]{3 fluid.jpg}\\[1pt]
    {\footnotesize (d) 3D extension}
  \end{minipage}
  \caption{Our preconditioner is designed for the SPD solves that bottleneck interactive
    physics simulation. (a) A representative result from the regime we target. (b) The
    hardest configuration in our benchmark distribution: a near-disconnected domain with
    $100\!\times$ density contrast, where every classical preconditioner we test loses
    interactive rate. (c) Visual showcase rendered from one of our solves. (d) Forward
    visualisation of the 3D extension of the same multiphase pressure-Poisson system; the
    architecture is dimension-agnostic and the quantitative 3D evaluation is a planned
    follow-up. Final renders to be supplied.}
  \label{fig:teaser}
\end{teaserfigure}
...
(Sec.~\ref{sec:background}). Figure~\ref{fig:matrices-and-rank}\,(left) shows that the
...
\begin{figure*}[t]
  ...
  \caption{The structural argument for the off-diagonal stack, in two views.
    ...
    learned preconditioner $M\!\approx\!A^{-1}$. $A^{-1}$ and $M$ share a rank-normalised
    colour scale and exhibit the same weak-admissibility block-and-tile pattern: full-rank
...
``compute, Tensor Cores'' row) and the ablation behaviour of \S\ref{sec:ablation}
...
prior is designed to capture. We chose our benchmark (Sec.~\ref{sec:results:benchmark}) to
be the most adversarial instance of this regime we could control: per-frame randomization
of barrier topology and contrast specifically populates these worst-case configurations.
...
pipelines --- and pick the 2D structured-grid instance specifically because it is the
\emph{hardest} variant of this family for local preconditioners. In 2D each node has only
$4$ neighbors, so an IC factor or SPAI row carries less local coupling information than in
3D, while the stiffness driven by density contrast is unchanged; on a structured grid the
classical Cartesian-grid baselines (IC, AMG) are at their strongest, so any gap we report
is against well-tuned competition. Comparable learned-preconditioner work in graphics
evaluates on problems of essentially the same order of
magnitude~\cite{Li2023learning,hausner2023neural,Yang2025sparse}.
Figure~\ref{fig:bench-frame} shows a representative frame.
...
\begin{figure}[t]
  ...
  \caption{End-to-end PCG solve time on the multiphase Poisson benchmark across five
    ...
    range; the closest GPU baseline (Diag/Jacobi) covers $\sim\!85$\,fps to
...
Figure~\ref{fig:scale_methods} reports per-frame mean$\pm$std solve time across $20$ test
...
Figure~\ref{fig:convergence} (figures-only page) traces how convergence actually looks
on a deliberately hard frame --- a closed cross-shaped barrier with stiff density
contrast chosen to maximise long-range coupling --- across five preconditioner families:
...
$1097$, $334$, $21$, $431$ for the five methods, top to bottom) match the
preconditioner-quality ordering, while the per-iteration cost ordering is reversed for
AMG and IC (\S\ref{sec:application}).
...
Figure~\ref{fig:probe-alignment} complements this view by showing probe-space alignment
...
its iteration count nearly triples at $N\!=\!8\,192$ (likely an optimisation issue on the
...
We probe robustness to two out-of-distribution axes within the multiphase Poisson
family: \emph{topology} (closed barriers withheld from training) and \emph{density
contrast} (training capped at $\rho_\text{heavy}\!\le\!25$, test frames drawn from
the full $[25,100]$ range). Table~\ref{tab:generalization} reports GPU results at
$N\!=\!4{,}096$ for the four most informative (train, eval) pairs.
...
high-contrast frames fall outside the learned eigenvalue cluster. Training on the
combined OOD regime (no closed barriers, low contrast) and evaluating on the hardest
test case (closed barriers, high contrast) yields the same $1.9\times$ speedup,
confirming that the two axes shift performance independently and that topology
generalization remains essentially free.

\begin{table}[h!]
\centering
\caption{Generalization across restricted training distributions at $N\!=\!4{,}096$
  (GPU; $\mathrm{rtol}\!=\!10^{-8}$, $20$ frames).
  \emph{Iters}: mean PCG iterations;
...
family we evaluate, ours is the only method that delivers convergence-quality solves
at interactive framerates across the full size range
...
\paragraph{Generalization across the regime.} The architecture depends on nothing
fluid-specific --- only a loose spatial ordering of degrees of freedom --- so the
recipe should transfer to other interactive modalities (see Fig.~\ref{fig:teaser}):
3D structured pressure-Poisson, unstructured 2D/3D meshes from real simulator
pipelines, and non-fluid SPD families like cloth and contact stiffness. Quantifying
that transfer, including a head-to-head against the sparse-approximate-inverse GNN
of Yang \emph{et al.}~\cite{Yang2025sparse} on a shared benchmark (in progress), is
the most important follow-up. Out-of-distribution sensitivity within the multiphase
Poisson family is reported in \S\ref{sec:results:generalization}.
...
ceiling at the cost of one extra pooling pass per layer, and is the most concrete
near-term architectural change we plan.
...
This is a scope statement, not a defect: it identifies the SPD families where this
recipe is the right tool.
...
\begin{figure}[!htbp]
  ...
  \caption{Residual propagation across preconditioner families on a deliberately hard
    ...
    Jacobi, IC, AMG, and ours. Each row shows $|r_k|$ on a shared log colour scale at
    five iteration snapshots; row labels give each method's iteration count to
    $\mathrm{rtol}\!=\!10^{-8}$. The $k\!=\!1$ column is the key claim: Jacobi and IC
